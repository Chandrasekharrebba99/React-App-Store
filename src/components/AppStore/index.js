...
import ProjectItem from '../AppItem'
import './index.css'
...
class AppStore extends Component {
  state = {activeTabID: tabsList[0].tabId, inputValue: ''}
  ...
  getFilterdItems = () => {
    ...
    const filterdItems = appsList.filter(
      arr =>
        arr.category === activeTabID &&
        arr.appName.toLowerCase().includes(inputValue.toLowerCase()),
    )
    return filterdItems
  }
  ...
  render() {
    ...
    const projectItems = this.getFilterdItems()
        ...
        <div>
          <ul>
            {projectItems.map(eachItem => (
              <ProjectItem key={eachItem.appId} allItems={eachItem} />
            ))}
          </ul>
        </div>
      </div>
